...
private:
	...
	vector <vector <pair <int, double> > > neigh;//neighbor lists
...
public:
	...
	void add_node(bool other_side=true){
		nnodes++;
		if (! other_side){
			typeA++;
			if(nnodes != typeA){
	...
	void remove_node(int x){
		...
		/* if already removed or invalid */
		if (x > nnodes || x < 0 || narity[x] == 0){
			throw new exception();
		}
		/* remove influence from neighbor nodes */
		for (int i = 0; i < nnodes; ++i){
			if (i == x){continue;}
			it = myfind(neigh[i].begin(),neigh[i].end(),x);
			/* if not a neighbor of x */
			if (it == neigh[i].end()){continue;}
			narity[i] -= it->second;
			if (x > typeA){
				neigh[i].erase(it);
			} else {
		...
		/* remove arity from this node */
		for (auto it = neigh[x].begin(); it != neigh[x].end(); ++it){
			it->second = 0;
		}
		tarity -= 2 * narity[x];
		narity[x] = 0;
	...
	void handle_unate(int x, bool pos_unate=true){
		// TODO?
	}

	void handle_refine(int x, vector<int> new_clause){
		// TODO?
	}
